...
def tail2angles(tailfit, fraction = .2, direction = 'down'):
    ...
    for i,fit in enumerate(tailfit):
        ...
        if direction == 'left':
            results[i] = math.degrees(numpy.arctan2(-temp[1],temp[0])) # using this line when tail points left
        elif direction == 'down':
...
def tail2sumangles(tailfit,direction='down'):
    ...
    for i,fit in enumerate(tailfit):   ############tag###########
        temp = fit[1:,:]-fit[:-1,:] ###temp is a list contains all the vectors from one dots to another
        # Note. -1 is the second last item

        if direction == 'left':
            results[i] = numpy.mean(90-numpy.degrees(numpy.arctan2(-temp[:, 0], temp[:, 1])))
        elif direction == 'down':
            results[i] = numpy.mean(-numpy.degrees(numpy.arctan2(temp[:, 0], temp[:, 1])))
...
def tail2sumangles_proximal(tailfit,direction='down'):
    # Note. this function calculate the vector degree between each fitted points and return the mean value of all the angles in each frame
    # Note. returned results is a list with the total items corresponds to each frame, items value is the mean of tail angle
...
def tail2sumangles_distal30(tailfit,direction='down'):
    # Note. this function calculate the vector degree between each fitted points and return the mean value of all the angles in each frame
    # Note. returned results is a list with the total items corresponds to each frame, items value is the mean of tail angle
